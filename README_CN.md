...
| 时区 | `TZ=Asia/Shanghai` |
| 数据库地址 | `POSTGRES_HOST` — 必填，无默认值 |
| 数据库端口 | `POSTGRES_PORT=5432` |
| 数据库名称 | `POSTGRES_DATABASE=postgres` |
| 数据库用户 | `POSTGRES_USER=postgres` |
| 数据库密码 | `POSTGRES_PASSWORD` — 必填，无默认值，输入时隐藏 |
...
- `/config` 保存应用配置、缩略图、预览和缓存；`/upload` 保存手机备份的照片和视频。两者都需要持久化和可写权限。
